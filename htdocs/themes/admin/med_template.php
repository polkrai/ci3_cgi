...
<body>
	...
	<?php $id_session  = ($this->input->get_post('id_sess') && $this->input->get_post('id_sess') != "")?$this->input->get_post('id_sess'):$this->session->userdata('id_sess');?>
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
        <div class="container-fluid">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="<?php echo site_url("Med/index?id_sess={$id_session}&date_select=".date('Y-m-d')); ?>"><b><?php echo $this->settings->site_name_med; ?> </b></a>
            </div>
            <div id="navbar" class="navbar-collapse collapse">
                ...
                <ul class="nav navbar-nav">	
                	...
                	<li class="dropdown <?php echo ($this->router->fetch_method() == 'form_med') ? 'active' : ''; ?>">
		                ...
		                <ul class="dropdown-menu">
							<li class="<?php echo (uri_string() == 'Med/form_med') ? 'active' : ''; ?>">
								<a id="form_med_add" href="<?php echo site_url("Med/form_med?date_select={$date_select}&id_sess={$id_session}");?>" ><?php echo 'บันทึกรายชื่อแพทย์ออกตรวจ'; ?></a>
							</li>
                	...
                	<li class="dropdown <?php echo ($this->router->fetch_class()  == 'Appointment') ? 'active' : ''; ?>">
		                ...
		                <ul class="dropdown-menu">
							<li class="<?php echo (uri_string() == 'Appointment/index') ? 'active' : ''; ?>"><a id="form_app_add" href="<?php echo site_url("Appointment/index/{$date_select}?date_select={$date_select}&id_sess={$id_session}");?>"><?php echo 'บันทึกนัดหมายผู้ป่วย'; ?></a></li>
							<li class="<?php echo (uri_string() == 'Appointment/') ? 'active' : ''; ?>"><a id="form_app_summary" href="<?php echo site_url("Appointment/summary/{$date_select}?&id_sess={$id_session}");?>"><?php echo 'สรุปการบันทึกนัดหมาย'; ?></a></li>
							<li class="<?php echo (uri_string() == 'Appointment/') ? 'active' : ''; ?>"><a id="form_app_his" href="<?php echo site_url("Appointment/history/{$date_select}?&id_sess={$id_session}");?>"><?php echo 'สรุปนัดหมายแบบแสดงรายชื่อ'; ?></a></li>
							<li class="<?php echo (uri_string() == 'Appointment/') ? 'active' : ''; ?>"><a id="form_app_con" href="<?php echo site_url("Appointment/conclube/{$date_select}?&id_sess={$id_session}");?>"><?php echo 'สรุปนัดหมายแบบแสดงรวมยอด'; ?></a></li>
		                </ul>
	                </li>     
                </ul>
...
function get_history(date_select) {
	...
	$.ajax({type: 'POST',
			url: '<?php echo site_url("Appointment/history/{$date_select}?&id_sess={$id_session}");?>',
			dataType : 'html',
...
setInterval(function(){
	...
	$.post(url_check_session, {user_id: '<?php echo $this->session->userdata('user_id')?>', id_sess:'<?php echo $id_session?>'}, function(result) {
...
function get_ajax_data(site_url, jdate) {
	...
	$.ajax({
		type: 'POST',
		url: site_url + '/' + jdate + '?id_sess=<?php echo $id_session?>&date_select=' + jdate,
...
function appointmentEdit(appointment_id, patient_name) {
	...
	winAppointment.attachURL('<?php echo site_url("Appointment/form/")?>' + appointment_id + '?id_sess=<?php echo $id_session?>&patient_name=' + patient_name + '&edit=true');
...
var appointmentPrint = function (appointment_id) {
	...
	var url = '<?php echo site_url("Appointment/print_app/")?>' + appointment_id + '?&id_sess=<?php echo $id_session?>';
