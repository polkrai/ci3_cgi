...
class Student extends REST_Controller {
    ...
    public function student_get(){
        ...
        $this->load->model('Mdl_Student', 'Model_student');
        $user_id = $this->get('id');
        
        // If the id parameter doesn't exist return all users
        if($user_id === NULL){
	        
            $users = $this->Model_student->get_many_by(array('status' =>  array('active', 'inactive')));
            if($users){
        ...
        $user = $this->Model_student->get_by(array('user_id' => $user_id, 'status' => array('active', 'inactive')));
    ...
    public function student_post(){
        ...
        if($this->form_validation->run('student_post') !=false){
            ...
            $this->load->model('Mdl_users', 'Model_student');
            $exists = $this->Model_student->get_by(array('email' => $this->post('email')));
            ...
            $user_id = $this->Model_student->insert($data);
            if(!$user_id){
    ...
    public function users_put(){
        ...
        if(isset($user['user_id'])){
            ...
            if($this->form_validation->run('user_put') !=false){
                ...
                $this->load->model('Mdl_student', 'Model_student');
                $safe_email = !isset($data['email']) || $data['email'] == $user['email'] || !$this->Model_student->get_by(array('email' => $data['email']));
                ...
                $updated = $this->Model_student->update($user_id, $data);
                if(!$updated){
    ...
    public function users_delete(){   
        ...
        if(isset($user['student_id'])){
            ...
            $data['status'] = 'deleted';
            $deleted = $this->Model_student->update($user_id, $data);
