...
class Sessions extends MX_Controller {
    ...
    public function set_session ($id_sess=NULL) {
	    ...
	    if ($id_sess) {
            ...
            if($this->mdl_sessions->auth_session($id_sess)) {
            	
            	//redirect("http://{$_SERVER['SERVER_NAME']}/nano");
            	
            	//header("Location: http://{$_SERVER['SERVER_NAME']}/nano");
    ...
    public function check_session () {
	    ...
	    if ($this->mdl_sessions->auth_check_session($this->input->get_post('user_id'))) { // && $this->session->userdata('logged_in')
