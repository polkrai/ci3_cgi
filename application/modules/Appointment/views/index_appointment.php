...
<table style="width:100%" cellpadding="5" cellspacing="5" >
	...
	<thead>
		...
		<th style="width: 25%; padding-bottom: 5px; padding-right: 5px;">
			<table id="table-25" class="easyui-datagrid" style="width:100%;height:350px" singleSelect="true" loadMsg="" url="<?php echo site_url("Appointment/load_data_queue_other/25?date_select={$date_select}&id_sess={$this->input->get_post("id_sess")}");?>" title="ยาเดิมส่งมาหลังพบแพทย์" rownumbers="true" method="get" > <!-- pagination="true" pageSize:10 -->
				<thead>
					<tr>
		...
		<th style="width: 25%; padding-bottom: 5px; padding-right: 5px;">
			<table id="table-0" class="easyui-datagrid" style="width:100%;height:350px" singleSelect="true" loadMsg="" url="<?php echo site_url("Appointment/load_data_queue_other/0?date_select={$date_select}&id_sess={$this->input->get_post("id_sess")}");?>" title="หน่วยงานอื่นส่งมาหลังพบแพทย์/ค้นหา" rownumbers="true" method="get" > <!-- pagination="true" pageSize:10 -->
				<thead>
					<tr>
						<th field="time_th" align="center" width="50">เวลา</th>
						<th field="patient_name" width="140">ชื่อ สกุล</th>
	...
	<?php echo ($row == 1)?"<thead><tr>":NULL?>
		...
		<th style="width: 25%; padding-bottom: 5px; padding-right: 5px;">
			<table id="table-<?php echo $med_station->station_id?>" class="easyui-datagrid" style="width:100%;height:350px" singleSelect="true" loadMsg="" url="<?php echo site_url("Appointment/load_data_queue/{$med_station->station_id}?date_select={$date_select}&id_sess={$this->input->get_post("id_sess")}");?>" title="<?php echo $med_station->station_name;?> <?php echo nbs(3);?> <?php echo $fullname;?>" rownumbers="true" method="get" > <!-- pagination="true" pageSize:10 -->
				<thead>
					<tr>
						<th field="time_th" align="center" width="50">เวลา</th>
						<th field="patient_name" width="140">ชื่อ สกุล</th>
...
function appointmentAdd (vn_id, queue_id, appointment_id, patient_name) {
	...
	winAppointment.attachURL('<?php echo site_url("Appointment/form/")?>' + appointment_id + '?vn_id=' + vn_id + '&queue_id=' + queue_id + '&patient_name='+ patient_name +'&date_select=<?php echo $date_select?>');
...
function appointmentAddOld (vn_id, queue_id, appointment_id, patient_name) {
	...
	$('#window-add').window('refresh', '<?php echo site_url("Appointment/form/")?>' + id + '?vn_id=' + vn_id + '&queue_id=' + queue_id + '&patient_name='+ patient_name +'&date_select=<?php echo $date_select?>');
...
function searchHN() {
	...
	var date_select = '&date_select=' + $('#datepicker-navbar').val();
		date_select+= '<?php echo "&id_sess={$this->input->get_post("id_sess")}"?>';
...
function reload_graid () {
	...
	//station_id
	var date_select = '?date_select=<?php echo $date_select?>';
		date_select+= '<?php echo "&id_sess={$this->input->get_post("id_sess")}"?>';
