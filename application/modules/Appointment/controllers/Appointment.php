...
class Appointment extends Med_Controller {
	...
	public function index ($date_select = NULL) {
		...
		$date_select = ($date_select)?$date_select:$this->input->get_post("date_select");

		if(!$date_select || $date_select == ""){
		...
		$content_data = array('med_stations' => $this->mdl_med_station->result(), 'form_search' => 'search_hn', 'date_select' => $date_select);
	...
	public function form ($appointment_id = NULL) {
		...
		$chk_holiday = "SELECT holiday_date
						FROM jvkk.sys_holiday
						WHERE holiday_date >= '{$years}-01-01'/*CAST(holiday_date AS text) LIKE '{$years}%'*/
						AND holiday_status = '1'
						ORDER BY holiday_date";

		$chk_query = $this->db->query($chk_holiday);

		if ($chk_query->num_rows() > 0) {

			foreach ($chk_query->result() as $value) {
				
				$value_holiday.= "\"{$value->holiday_date}\",";
			}
		}

		$value_holiday = substr($value_holiday, 0, -1);

		$data['value_holiday'] = $value_holiday;
	...
	public function summary ($date = NULL) {
		...
		$this->db->where('neural_appointment.deleted', 'f');
		$this->db->where('doctors.group_id', 1);
	...
	public function print_app ($appointment_id = NULL) {
		...
		if(count($data['appointment']) > 0) {
			...
			echo "<center style=\"font-size:24px;\">ยังไม่มีรายการนัดหมาย</center>";
		}
	}
