...
<?php $date_select = ($this->input->get_post('date_select'))?$this->input->get_post('date_select'):date('Y-m-d')?>
	...
	<table class="table table-striped">
   		...
   		<?php $i=1; foreach($stations as $station):?>	
   			<tr>
   				...
   				<td style="width: 30%;">
	   				<?php echo form_dropdown("doctor_id[{$station->station_id}]", $users, $this->mdl_model->get_last_date($station->station_id, $date_select), 'class="form-control" style="font-size:14px;"');?>
	   			</td>
   			</tr>
   		<?php $i++; endforeach;?>
   		</tbody>
